...
_productionpath = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_m2ku_cn/配置文件/各项目包配置文件/production/'))
_mossespath = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_m2ku_cn/配置文件/各项目包配置文件/test/'))

_boss_branch_test = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_branch_cn/配置文件/test/'))
_boss_branch_beta = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_branch_cn/配置文件/beta/'))
_boss_branch_cn = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_branch_cn/配置文件/moxiancn/'))
_boss_branch_online = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_branch_cn/配置文件/production/'))

_boss_master_test = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_master_cn/配置文件/test/'))
_boss_master_beta = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_master_cn/配置文件/beta/'))
_boss_master_cn = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_master_cn/配置文件/moxiancn/'))
_boss_master_online = os.path.abspath(os.path.join(os.path.dirname(__file__), 'workspace/moxian_boss_master_cn/配置文件/production/'))
...
_codeCNBoss_v202 = os.path.abspath(os.path.join(os.path.dirname(__file__), 'compilePath/boss_branch/moxian_boss_branch_cn'))
...
_log_file = os.path.abspath(os.path.join(os.path.join(os.path.dirname(__file__),'compileLog'),'compileforjenkins_cn.log'))
...
class writeexcel(object):
    ...
    def GetrowsAndData(self,modelname):
        ...
        datasList = []
        if lineNum < 1:
            data = ['包名','申请时间','后台包负责人','测试环境','beta是否发包','CN是否发包','是否打了线上包','上线日期','beta是否验证','beta验证结果','解决问题','是否涉及前端','是否涉及数据库','问题跟踪人','备注']
                    ...
                    for c in range(15): 
                        ...
                        if row == 0:
                            ...
                            if c == 0:
                                ...
                                value = str(datasList[datasListIndex][5])+'(real time:'+time.strftime('%Y/%m/%d %H:%M',time.localtime())+')'
...
class getexcel(object):
    ...
    def _getsheet(self):
        ...
        if sheetName not in largestSheet:
            logging.error('Error:No sheet named startswith:%s,the latest table is %s, please sure,and progaram exit!' % (sheetName,str(largestSheet)))
            raise ValueError('Build Failed!')
                ...
                if self.appflag:
                    ...
                    for f in files:
                        ...
                        if m_name not in model_name_list and m_name != '':
                            model_name_list.append(m_name.strip())
        logging.info('Excel models that need compiled:%s----and models that don\'t need compile:%s' % (model_name_list,models_no))
                ...
                if i.startswith('mo_'):
                    ...
                    for md in mo_dirlist:
                        ...
                        if curtime - md_mtime >= 1296000:
                            ...
                            logging.info('Delete the backup files that time greater than 15 days:'+destpath+'/'+i+'/'+md)
    ...
    def scpsend(self):
        ...
        if type(module) == types.StringType:
            ...
            try:
                ...
                CMD = "scp -P {scpport} {warpackage} root@{scpip}:{scppath}/{model}".format(scpport=scpport, warpackage=modulewar, scpip=scpip, scppath=scppath,model=module)
            ...
            for m in module:
                ...
                try:
                    ...
                    if m == "mo_game_mining":m = "mo_mining"
                    modulewar = m +'.war'
                    CMD = "scp -P {scpport} {warpackage} root@{scpip}:{scppath}/{model}".format(scpport=scpport, warpackage=modulewar, scpip=scpip, scppath=scppath,model=m)
                    logging.debug('excute cmd:%s,transfering file,please wait...' % CMD)
                    pp = subprocess.Popen(CMD,shell=True,stderr=subprocess.PIPE,stdout=subprocess.PIPE)
                    stdout, stderr = pp.communicate()
                    pp.wait()
...
def code_modoule_select(*args):
    ...
    modoule_env_dict = {'1':'M2','2':'Boss_master','3':'Boss_v2.0.3','4':'Test_M2','5':'Test_Boss_master','6':'Test_Boss_v2.0.3'}
...
if __name__ == '__main__':
    ...
    if 'boss' in args[1]:
        if main_arg1 != 'mo_boss':
            logging.warning("selected boss,so script arg2 must be 'mo_boss'(your given '%s'),and i will set it as 'mo_boss'" % main_arg1)
            main_arg1 = 'mo_boss'
    main_arg3 = args[3]
    ...
    if main_arg7 not in ['EN','EY']:
        ...
        raise ValueError('Build Failed!')
    logging.info("script's args:arg1=%s,arg2=%s,arg3=%s,arg4=%s,arg5=%s,arg6=%s" % (args[1],args[2],args[3],args[4],args[5],args[6]))
